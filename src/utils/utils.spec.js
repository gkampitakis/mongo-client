const { extractUniqueValues, isEmptyObject, objectEquality, objectID, stripObject } = require('./utils');
...
describe('Utils', () => {
  ...
  describe('Function extractUniqueValues', () => {
    it('Should return the paths with unique values', () => {
      ...
      expect(extractUniqueValues((object))).toEqual(['test.name', 'test2']);
    });
    ...
    it('Should return nothing if no required', () => {
      ...
      expect(extractUniqueValues(object)).toEqual([]);
    });
  });
